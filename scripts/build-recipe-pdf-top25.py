...
OUT = os.path.join(PUB, "pdf", "haowei-top25.pdf")
...
recipes_all = json.load(open(DATA, encoding="utf-8"))

# ---------- Top 25 子集（日常高频家常菜，按 slug 白名单） ----------
TOP25_SLUGS = [
    "tomato-and-egg", "egg-fried-rice", "braised-tofu", "home-style-tofu",
    "scallion-noodles", "tomato-egg-noodles", "yangchun-noodles",
    "egg-drop-soup", "tomato-egg-drop-soup", "winter-melon-soup",
    "beef-and-broccoli", "green-pepper-beef", "onion-beef-stir-fry",
    "potato-beef-stew", "cashew-chicken", "coke-chicken-wings",
    "hongshao-chicken", "soy-chicken-wings", "pan-fried-chicken",
    "hongshao-pork", "sweet-sour-pork", "sweet-sour-ribs",
    "pepper-pork", "vinegar-cabbage", "spicy-potato-shreds",
]
recipes = [r for r in recipes_all if r.get("slug") in TOP25_SLUGS]
# 保持 TOP25_SLUGS 顺序
slug_rank = {s: i for i, s in enumerate(TOP25_SLUGS)}
recipes.sort(key=lambda r: slug_rank.get(r.get("slug"), 999))
print("Top25 subset:", len(recipes), "recipes")
...
class Doc(BaseDocTemplate):
    ...
    def onPage(canvas, doc):
        ...
        canvas.drawString(2*cm, 1.1*cm, "好味 HǎoWèi · Top 25 家常菜（首发免费版）")
...
doc = Doc(OUT, pagesize=A4, title="好味 Top 25 家常菜电子书（免费）", author="HǎoWèi")
...
story.append(Paragraph("Top 25 家常菜电子书", st_cover_sub))
story.append(Paragraph("HǎoWèi Top 25 Homestyle Chinese Recipes — Free eBook", st_cover_en))
...
story.append(Paragraph("[ 免费电子书 ] 下载即读", st_cover_badge))
story.append(Spacer(1, 0.5*cm))
story.append(Paragraph("25 道家常菜　·　中英双语　·　公制 + 美制对照　·　附火候判断图解", st_cover_meta))
story.append(Paragraph("2026 年 8 月 · 电子书第 1 版", st_cover_meta))
