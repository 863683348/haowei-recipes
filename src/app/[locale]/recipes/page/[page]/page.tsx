...
import { filterRecipes, cuisineOptions, recipes as allRecipes } from "@/data/recipes";
...
export default async function RecipesPagePaged({ params, searchParams }: Props) {
  const { locale, page } = await params;
  ...
  /** 分页链接（路径式，保留筛选参数；第 1 页回 /recipes） */
  ...
  function pageButtons(): (number | "...")[] {
    ...
    if (totalPages <= 7) {
      ...
      btns.push(1);
      if (cur > 3) btns.push("...");
      addRange(Math.max(2, cur - 1), Math.min(totalPages - 1, cur + 1));
      if (cur < totalPages - 2) btns.push("...");
      btns.push(totalPages);
    }
    return btns;
  }

  // ItemList JSON-LD（只含当前页）
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        ...
        {/* 筛选条 */}
        <form className="mt-6 flex flex-wrap items-center gap-2" method="get" action={localizePath("/recipes", loc)}>
          <select
            name="cuisine"
                ...
                {pageButtons().map((b, i) =>
                  b === "..." ? (
                    ...
                    <Link
                      key={b}
                      href={pageHref(b)}
                      aria-current={b === cur ? "page" : undefined}
